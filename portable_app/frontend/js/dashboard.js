...
async function loadDashboardData() {
    try {
        // 통계 데이터 로드
        const statsResponse = await fetch(`${API_BASE_URL}/statistics/summary`);
        if (!statsResponse.ok) {
...
function getLevelBadge(level) {
    const badgeMap = {
        'EXACT': '<span class="badge badge-danger">정확일치</span>',
        'LEVEL_3-4': '<span class="badge badge-warning">3-4자리</span>',
        'LEVEL_4-1': '<span class="badge badge-warning">4-1자리</span>',
        'LEVEL_5-X': '<span class="badge badge-warning">5-X자리</span>'
    };

    return badgeMap[level] || `<span class="badge badge-success">${level}</span>`;
}
